...
sed -nr "s/([[:alnum:]]+) ([[:alnum:]]+) ([[:alnum:]]+) ([-_[:alnum:]]+) ([^[:space:]]+)/\3/gp" < temp0.txt | sort | uniq -c > temp.txt
...
cat $here/html_components/hours_dist_header.html temp1.html $here/html_components/hours_dist_footer.html > hours_dist.html
